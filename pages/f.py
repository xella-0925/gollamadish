...
if uploaded_file is not None:
    ...
    for no_of_tests in range(no_of_imgs):
        Tx = st.number_input(f"Tx (Image {no_of_tests+1}):", min_value=-cols, max_value=cols, step=1, value=0)
        Ty = st.number_input(f"Ty (Image {no_of_tests+1}):", min_value=-rows, max_value=rows, step=1, value=0)
